...
export default function StressTestPage() {
  ...
  return (
    <div className="space-y-5 max-w-7xl mx-auto">
      <div>
        <h1 className="font-mono text-lg font-bold uppercase tracking-wider text-[color:var(--text)] font-mono">
          Stress Test Desk
        </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="space-y-4">
          <div className="bg-[var(--surface)] border p-4" style={{ borderColor: "var(--border)" }}>
            <p className="font-mono text-xs font-bold uppercase text-[color:var(--text)] mb-3">Select Scenario</p>
            <ScenarioPicker
              selected={scenario}
          ...
          <div className="bg-[var(--surface)] border p-4" style={{ borderColor: "var(--border)" }}>
            ...
            <p className="font-mono text-[9px] text-[color:var(--muted-2)] mb-2 uppercase">
              Symbol → weight (0–1). Must sum to 1.0
            </p>
            <textarea
              aria-label="Custom stress test scenario"
              value={portfolioJson}
              onChange={(e) => setPortfolioJson(e.target.value)}
              rows={8}
              className="w-full bg-[var(--surface)] border px-3 py-2 text-xs font-mono text-text focus:outline-none focus:border-emerald-500/60 focus:ring-1 focus:ring-emerald-500 resize-none"
        ...
        <div className="lg:col-span-2">
          ...
          {loading && (
            <div className="h-64 bg-[var(--surface)] border flex items-center justify-center animate-pulse" style={{ borderColor: "var(--border)" }}>
              <p className="font-mono text-xs text-[color:var(--muted-2)] uppercase">Fetching historical crash data…</p>
            </div>
          )}
          {!result && !loading && !error && (
            <div className="h-64 bg-[var(--surface)] border flex items-center justify-center" style={{ borderColor: "var(--border)" }}>
              <div className="text-center">
                <p className="font-mono text-xs font-bold uppercase text-[color:var(--muted-2)] mb-1">Select a scenario and run the stress test</p>
